...
interface OnCompoundDrawableTouchListener : View.OnTouchListener {
    ...
    override fun onTouch(v: View?, event: MotionEvent?): Boolean {
        ...
        if (onTouchIndexedDrawable != null) {
    ...
    private fun getDrawableBounds(index: Int, drawable: Drawable, view: TextView): Rect {
        val drawableLeftWidth = view.compoundDrawables[LEFT]?.takeIf { it.isVisible }?.intrinsicWidth ?: 0
        val drawableRightWidth = view.compoundDrawables[RIGHT]?.takeIf { it.isVisible }?.intrinsicWidth ?: 0
